...
| # | Status | Task |
|---|--------|------|
| POSIX-1 | `[~]` | `zamak-cli` — uses only `std::fs`/`std::io`; CI exercises Linux + macOS; FreeBSD in CI matrix pending (Forgejo runner availability) |
| POSIX-2 | `[~]` | CI matrix — Linux x86-64, Linux AArch64, macOS AArch64 in `test` job; FreeBSD still pending |
...
| # | Status | Task |
|---|--------|------|
| TEST-1 | `[~]` | Unit tests for `zamak-core` — 119 lib tests + 2 doctests + 7 proptests passing (vs ≥80% coverage target; `cargo-llvm-cov` measurement pending) |
| TEST-2 | `[~]` | Miri scaffolding — `.cargo/config.toml` with `MIRIFLAGS` + `cargo miri-test` alias; `#[cfg(miri)]` stubs for all asm blocks; nightly Miri toolchain not installed in local env |
...
| REL-7 | `[~]` | `zamak` CLI binary — `cli-binaries` matrix job (Linux x86-64, macOS AArch64); FreeBSD pending |
...
- **External toolchains** — `cargo-llvm-cov` coverage measurement (TEST-1), nightly Miri toolchain (TEST-2) — 2 items
...
- **FreeBSD CI runner** — POSIX-1 / POSIX-2 (2 items)
