...
| M1-16 | `[~]` | End-to-end BIOS Limine-Protocol kernel boot under QEMU. UEFI case `[✓]`. BIOS work in-progress: (a) `build-images.sh` now builds `zamak-stage1` + `zamak-bios`, `objcopy`s them to raw binaries, assembles a real MBR + stage2 + FAT32-partition disk at `target/zamak-bios.img` — image layout verified (correct stage2 LBA/size at MBR offsets 440/444, sfdisk partition table at 446, boot signature at 510); (b) `zamak-stage1` gained the `build.rs` it was missing so rust-lld now consumes `linker.ld`; (c) `zamak-bios`'s `smp`/`trampoline` modules are feature-gated OFF by default — the AP trampoline's real-mode asm uses 16-bit absolute relocations that rust-lld can't place once `.trampoline` lands past 64 KiB in the ELF, a position-independent rewrite is out of M1-16 scope. Remaining: QEMU boot trace shows stage1 executes and returns from `INT 13h` but serial stays silent (BIOS `INT 10h` teletype routes to display, not COM1). Next steps for whoever picks this up: instrument stage1 + zamak-bios entry asm with direct `outb 0x3F8` writes so boot progress shows on serial, then iterate on the stage1→zamak-bios handoff and the FAT32/partition-table readers in `zamak-bios` which have never been exercised end-to-end. `bios-boot-smoke` remains removed from the `boot-smoke` suite. |
