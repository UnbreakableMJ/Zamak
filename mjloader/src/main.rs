...
use alloc::vec;
use alloc::boxed::Box;
use libmjboot::config;
use libmjboot::elf;
use libmjboot::protocol;
        ...
        match req.id {
            protocol::BOOTLOADER_INFO_ID => {
                let response = Box::leak(Box::new(protocol::BootloaderInfoResponse {
                    name: Box::leak(Box::new("mjboot\0")).as_ptr() as u64,
                    version: Box::leak(Box::new("0.1.0\0")).as_ptr() as u64,
                }));
