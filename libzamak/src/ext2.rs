...
use crate::fs::{FileSystem, BlockDevice, FileEntry, FileType, Error};
use alloc::vec::Vec;
use alloc::vec;
...
impl<'a> Ext2<'a> {
    ...
    fn read_block(&self, block_id: u32, buffer: &mut [u8]) -> Result<(), Error> {
        ...
        self.device.read_sectors(lba, sectors_per_block as usize, buffer)
    }
    ...
    fn get_inode(&self, inode_num: u32) -> Result<Inode, Error> {
        if inode_num == 0 { return Err(Error::FileNotFound); }
        ...
        let _inode_size = self.superblock.rev_level.max(128) as u32; // Simplified
    ...
    fn read_inode_data(&self, inode: &Inode, offset: u32, buffer: &mut [u8]) -> Result<usize, Error> {
    ...
    fn find_in_dir(&self, dir_inode: &Inode, name: &str) -> Result<u32, Error> {
        ...
        while offset < size {
            ...
            if block_idx >= 12 { break; } // Only search first 12 blocks of dir for now
            
            let block = dir_inode.block[block_idx as usize];
            if block == 0 { offset += block_size; continue; }
            ...
            while ptr < end {
                ...
                if header.inode == 0 { break; } // End/Unused
...
impl<'a> FileSystem for Ext2<'a> {
    fn find_file(&self, path: &str) -> Result<FileEntry, Error> {
        ...
        let inode = self.get_inode(current_inode_num)?;
        let file_type = if (inode.mode & 0xF000) == 0x4000 { FileType::Directory } else { FileType::File };
