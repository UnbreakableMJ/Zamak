...
impl<'a> PsfFont<'a> {
    ...
    pub fn draw_char(&self, canvas: &mut Canvas, x: u64, y: u64, c: char, fg: Color, bg: Option<Color>) {
        let char_h = self.header.char_size as u64;
        let char_w = 8; // PSF1 is always 8 pixels wide
        
        // Basic ASCII mapping
        let char_idx = if c.is_ascii() { c as usize } else { '?' as usize };
    ...
    pub fn draw_string(&self, canvas: &mut Canvas, x: u64, y: u64, s: &str, fg: Color, bg: Option<Color>) {
        let mut cur_x = x;
        for c in s.chars() {
